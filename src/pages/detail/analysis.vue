    ...
    getPrice () {
      ...
      //ajax请求总价
       axios.post('api/getPrice',reqParams)    
        .then((res) => {
             this.price = res.data.number//mockjs作用生成随机price
        })
        .catch((error) => {
            console.log(error)
        })
     ...
     confirmBuy () {
      ...
      let createParams = {
       ...
       axios.post('api/createOrder',createParams)    
        .then((res) => {
          this.orderId = res.data.orderId
          this.isShowCheckOrder = true //显示支付结果验证弹框
          this.isShowPayDialog = false //关闭当前弹框
        })
        .catch((error) => {
            console.log(error)
        })
